...
from main import get_final_results, main_functions, main_interface
...
@patch("builtins.input", side_effect=["2", "CANCELED", "", "", "", "", "42"])
...
@patch("builtins.input", side_effect=["0", "5", "1", "CAN", "3", "executed", "", "", "", "", "23"])
...
class TestGetResults(unittest.TestCase):
    @patch("sys.stdout", new_callable=StringIO)
    def test_get_final_results_no_transactions(self, mock_stdout) -> None:
        """Проверяет работу функции get_final_results"""
        # transactions_for_generate - фикстура из модуля conftest.py
        transactions_list = [{}]
        get_final_results(transactions_list)
        self.assertIn("Не найдено ни одной транзакции, подходящей под ваши условия фильтрации", mock_stdout.getvalue())
